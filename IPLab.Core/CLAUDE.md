...
- **Every operator class:** add `/// <seealso href="https://github.com/yoavmil/IPLab/blob/master/docs/OPERATORS.md#anchor">Operator reference</seealso>` pointing to its section in OPERATORS.md. The anchor is the heading text lowercased (e.g. `## DistortionCalibration` → `#distortioncalibration`). Place this after any OpenCV `<seealso>` tags, or directly after `</summary>` when there are none.
...
**Rule: before bumping `<Version>` in `IPLab.Core.csproj` and packing a new NuGet release, build all scripts that depend on the package to confirm the public API they use has not broken. At minimum run `dotnet build scripts\Waviness\Waviness.csproj -c Release` and fix any compilation errors before publishing.**
